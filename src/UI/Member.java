...
public class Member extends JFrame {
	 ...
	 */
	public Member() {
		Object[] columnNames = {"아이디", "이름", "생년월일", " 성별 ", " 전화번호", "포인트"};
		...
		for (int i = 0; i < mlist.size(); i++) {
			...
			dto = mlist.get(i);
			dtm.addRow(new Object[] {dto.getId(), dto.getName(), dto.getBirth(), dto.getGen(), dto.getPhone(), dto.getPoint()});
		}
		...
		btnNewButton_1.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				dto1.setGen(table.getValueAt(row, 3).toString());
				dto1.setPhone(table.getValueAt(row, 4).toString());
		...
		btnNewButton_3.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				for (int i = 0; i < mlist.size(); i++) {
					MemberDTO dto = new MemberDTO();
					dto = mlist.get(i);
					dtm.addRow(new Object[] {dto.getId(), dto.getName(), dto.getBirth(), dto.getGen(), dto.getPhone()});
				}
			}
		});
